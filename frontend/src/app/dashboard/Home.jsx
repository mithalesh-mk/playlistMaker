  ...
  return (
    <SidebarInset>
      <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
            ...
            top2Data.map((playlist) => {
              return (
                ...
                <div className="w-full md:h-[400px] lg:h-[400px] xl:h-[450px]  group rounded cursor-pointer relative">
                  ...
                  {/* Heart Icon */}
                  <div className="rounded-full bg-black bg-opacity-50 w-10 h-10 justify-center items-center absolute top-2 right-16 
                                 flex opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 ease-in-out">
                    <HeartIcon className="text-white" />
                  </div>
              
                  {/* Share Icon */}
                  <div className="rounded-full bg-black bg-opacity-50 w-10 h-10 justify-center items-center absolute top-2 right-2
                                 flex opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 ease-in-out">
                    <Share2 className="text-white" />
                  </div>
              
                  {/* Category Label */}
                  <div className="absolute bottom-2 left-2 right-0 px-3 py-1 font-bold text-white/85 text-md uppercase w-fit 
                ...
                <div className="pt-2 flex gap-4 bg-black bg-opacity-50 rounded-b-xl">
                  <div>
                    <img src={`${playlist.user?.profilePic}`}  className="rounded-full w-10 h-10 md:w-14 md:h-14" />
                  </div>
                  <div>
                    <p className="md:text-2xl text-xl text-white/90 font-bold capitalize">{playlist.name}</p>
                    <p className="text-white/60 text-sm capitalize">{playlist.user?.username}</p>
                  </div>
                </div>
              </div>
        ...
        <div>
          <p className="md:text-4xl text-2xl font-semibold">Explore by Categories</p>
          <div className='flex gap-4  flex-wrap  mt-4'>
          {
            categoies.map((category) => {
              return <div key={category} className="bg-muted lg:py-3 lg:px-6 px-4 py-2 rounded-full cursor-pointer"> 
                
                <p className="md:text-lg text-base font-semibold uppercase">{category}</p>
              </div>
            })  
          }
          </div>
        ...
        <div className="grid auto-rows-min gap-4 mt-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {playlists.length > 0 &&
            playlists.map((playlist) => {
              return <div key={playlist._id} className="w-[100%] md:w-[90%]">
              ...
              <div className="pt-2 flex gap-4 bg-black bg-opacity-50 rounded-b-xl">
                <div>
                  <img src={`${playlist.user?.profilePic}`} width={62} className="rounded-full" />
                </div>
                <div>
                  <p className="text-md line-clamp-1 text-white/90 font-bold capitalize">{playlist.name}</p>
                  <p className="text-white/60 text-sm capitalize">{playlist.user?.username}</p>
                </div>
              </div>
            </div>
